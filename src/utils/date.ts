...
export function getArabicDate(date: Date, isPastMaghrib: boolean = false, autoAdjustment: number = 0) {
	const baseAdjustment = isPastMaghrib ? HIJRI_DATE_ADJUSTMENT + 1 : HIJRI_DATE_ADJUSTMENT;
	
	// Pastikan rumus ini ada:
	const finalAdjustment = baseAdjustment + autoAdjustment; 
	
	// Pastikan yang dimasukkan ke addDays adalah finalAdjustment
	const adjustedDate = addDays(date, finalAdjustment); 
